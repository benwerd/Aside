...
namespace IdnoPlugins\Aside {
    ...
    class Note extends \Idno\Common\Entity implements \Idno\Common\JSONLDSerialisable
    {
        ...
        function getURL()
        {
            ...
            if (!$this->getSlug() && ($this->getID())) {
                return \Idno\Core\Idno::site()->config()->url . 'aside/' . $this->getID() . '/' . $this->getPrettyURLTitle();
            } else {
        ...
        function saveDataFromInput()
        {
            ...
            if (!empty($body)) {
                ...
                // Make Note publish status aware
                ...
                if ($result = $this->publish($new)) {

                    $autosave = new Autosave();
                    $autosave->clearContext('aside');
                ...
                \Idno\Core\Idno::site()->session()->addErrorMessage(\Idno\Core\Idno::site()->language()->_('You can\'t save an empty aside.'));
            }
